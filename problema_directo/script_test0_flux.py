...
#Tsinc = []
#for i in range(nfrq):
    #freqs = np.append(freqs,flux_freqs[i])
    #Ts13 = np.append(Ts13,near_flux_receptor3[i]/near_flux_transmisor[i])
    #Ts14 = np.append(Ts14,near_flux_receptor4[i]/near_flux_transmisor[i])
...
#ax1.semilogx(frecuencia,S12inc,'-k')
#ax1.semilogx(frecuencia,S12cil,'-b')
#ax2 = fig2.add_subplot(122)
##conductividad = -epsc.imag*(eps0*2*pi*frec)
#ax2.semilogx(frec,-epsc.imag,'-b')
#plt.show()
...
##Dibujo el mapa de permitividad
#plt.figure()
#plt.imshow(N.log10(abs(eps_data.transpose())), interpolation='spline36', cmap='binary')
plt.show()
