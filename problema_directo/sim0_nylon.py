...
nfrec = 64
nant = 8
f_medida = 1e6*datos_cilindronylon[:,0]
...
cilindro1.epsr = 4.3 #permitividad relativa. Entre [10.0, 80.0]
cilindro1.sigma = 0.0
...
ax1.plot(Sinc_FINAL[0,:],'o-',label='Incidente, Tx: 1')#, f: '+str(f_medida[nfrec]/1e6)+' MHz')
...
#ax1.plot(S_nylon_FINAL[4,:],'^m--',label='Nylon, Tx: 5')#, f: '+str(f_medida[nfrec]/1e6)+' MHz')
#ax1.plot(S_nylon_FINAL[5,:],'+m--',label='Nylon, Tx: 6')
#ax1.plot(S_nylon_FINAL[6,:],'.m--',label='Nylon, Tx: 7')
ax1.set_ylim([-65, -25])
...
ax2.plot(10.0*np.log10(EzTr[1:]/7.5e4),'ob--',label='Nylon')#, f: '+str(f_medida[nfrec]/1e6)+' MHz')
#ax2.set_ylim([-65, -35])
